...
from liquidai_cli.utils.config import load_config, generate_random_string, extract_model_name, save_config
...
def launch(
    ...
    env_vars = {
        ...
        "DATABASE_URL": f"postgresql://{config['database']['user']}:{config['database']['password']}@liquid-labs-postgres:{config['database']['port']}/{config['database']['name']}"
    }
...
def purge(
    ...
    message = "This will remove all Liquid Labs components:\n" \
